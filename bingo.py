...
sorteado_na_cartela = [] # Essa, por via das dúvidas, mostrará os números que foram sorteados e que estão dentro da cartela do usuário.
numerosSorteados = []
...
while(prosseguir == 'S' and len(sorteado_na_cartela) < 5):
    sorteio = random.randint(1,10)
    while(sorteio in numerosSorteados):
         sorteio = random.randint(1,10)
    ...
    if len(sorteado_na_cartela) == len(suaCartela) - 1:
